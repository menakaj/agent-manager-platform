...
if [[ "$SKIP_KIND" == "true" ]]; then
    ...
    # Auto-connect devcontainer to kind network for seamless networking
    if [ -f /.dockerenv ] || grep -q docker /proc/1/cgroup 2>/dev/null; then
        if [[ "$VERBOSE" == "false" ]]; then
            echo "Connecting container to kind network..."
        else
            log_info "Detecting container environment, connecting to kind network..."
        fi

        # Get current container ID (use hostname which is container ID in Docker)
        CONTAINER_ID=$(hostname)

        if [ -n "$CONTAINER_ID" ]; then
            # Check if already connected to kind network
            NETWORK_CHECK=$(docker inspect "$CONTAINER_ID" --format='{{range .NetworkSettings.Networks}}{{.Name}} {{end}}' 2>/dev/null || echo "")

            if [[ "$NETWORK_CHECK" != *"kind"* ]]; then
                if docker network connect kind "$CONTAINER_ID" 2>/dev/null; then
                    if [[ "$VERBOSE" == "false" ]]; then
                        echo "✓ Connected to kind network"
                    else
                        log_success "Container connected to kind network"
                    fi
                else
                    if [[ "$VERBOSE" == "true" ]]; then
                        log_warning "Could not connect to kind network (may already be connected)"
                    fi
                fi
            else
                if [[ "$VERBOSE" == "true" ]]; then
                    log_info "Already connected to kind network"
                fi
            fi
        fi
    fi
...
# Verify OpenChoreo Build Plane is available
...
if [[ "${AUTO_PORT_FORWARD}" == "true" ]]; then
    if [[ "$VERBOSE" == "false" ]]; then
        ...
        log_info "Starting port forwarding using socat (OpenChoreo approach)..."
    fi

    # Use socat-based port forwarding function from install-helpers.sh
    if setup_amp_port_forwarding "openchoreo-local"; then
        if [[ "$VERBOSE" == "false" ]]; then
        ...
        if [[ "$VERBOSE" == "true" ]]; then
            log_warning "Port forwarding setup encountered issues (non-fatal)"
        fi
    fi
    echo ""
fi
...
echo "   • GitHub:           https://github.com/wso2/ai-agent-management-platform"
echo ""
