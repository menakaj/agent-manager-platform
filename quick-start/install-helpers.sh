...
AMP_CHART_VERSION="${AMP_CHART_VERSION:-0.0.10}"
BUILD_CI_CHART_VERSION="${BUILD_CI_CHART_VERSION:-0.0.10}"
OBSERVABILITY_CHART_VERSION="${OBSERVABILITY_CHART_VERSION:-0.0.10}"
...
setup_kind_cluster() {
    ...
    # Fix kubeconfig if running in container
    fix_kubeconfig_for_container "$cluster_name"
...
# When running in a container, localhost doesn't work for Kind clusters
fix_kubeconfig_for_container() {
    local cluster_name="${1:-openchoreo-local}"
    
    # Check if we're in a container
    if [ -f /.dockerenv ] || grep -q docker /proc/1/cgroup 2>/dev/null; then
        log_info "Detected container environment, fixing kubeconfig..."
        
        # Export kubeconfig with internal endpoint
        kind export kubeconfig --name "$cluster_name" --internal 2>/dev/null || {
            # Fallback: manually update the server URL
            local control_plane_ip=$(docker inspect "${cluster_name}-control-plane" \
                --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 2>/dev/null)
            
            if [ -n "$control_plane_ip" ]; then
                kubectl config set-cluster "kind-${cluster_name}" \
                    --server="https://${control_plane_ip}:6443" >/dev/null 2>&1
                log_success "Kubeconfig updated for container access"
            fi
        }
    fi
}
...
# ============================================================================
# PORT FORWARDING FUNCTIONS (OpenChoreo socat approach)
# ============================================================================
...
kill_port_process() {
    ...
    if [[ -n "$pids" ]]; then
        log_warning "Killing existing processes on port $port (PIDs: $pids)"
        kill -9 $pids 2>/dev/null || true
        sleep 1
    fi
}

# Start socat with verification
start_socat_verified() {
    local local_port="$1"
    local worker_node="$2"
    local node_port="$3"
    local description="$4"
    ...
    if ! check_port_available "$local_port"; then
        log_error "Port $local_port still in use after cleanup"
        return 1
    fi

    for attempt in $(seq 1 $max_retries); do
        log_info "Starting $description port-forward: $local_port -> $worker_node:$node_port (attempt $attempt/$max_retries)"

        # Start socat in background with proper options
        # fork: handle multiple connections
        # reuseaddr: allow quick restart
        # TCP-NODELAY: disable Nagle's algorithm for better responsiveness
        socat TCP-LISTEN:$local_port,fork,reuseaddr,bind=0.0.0.0 TCP:$worker_node:$node_port,nodelay &
        local socat_pid=$!

        # Wait a bit for socat to start
        sleep 2

        # Verify socat process is still running
        if ! kill -0 $socat_pid 2>/dev/null; then
            log_warning "$description socat process died immediately (attempt $attempt/$max_retries)"
            continue
        fi

        # Verify port is now in LISTEN state
        if lsof -i ":$local_port" -sTCP:LISTEN -t >/dev/null 2>&1; then
            log_success "$description port forwarding active (PID: $socat_pid)"
            return 0
        else
            log_warning "$description port not in LISTEN state, killing socat (attempt $attempt/$max_retries)"
            kill -9 $socat_pid 2>/dev/null || true
            sleep 1
        fi
    done

    log_error "Failed to start $description port forwarding after $max_retries attempts"
    return 1
}

# Setup port forwarding using socat (like OpenChoreo)
setup_amp_port_forwarding() {
    local cluster_name="${1:-openchoreo-local}"
    local worker_node="${cluster_name}-worker"

    log_info "Setting up port forwarding using socat..."
    ...
    if ! command_exists lsof; then
        log_warning "lsof not found, installing basic port checking..."
        # Fallback: just kill all socat processes
    ...
    # Console (3000)
    log_info "Finding Console NodePort..."
    local nodeport_console
    for i in {1..30}; do
        nodeport_console=$(kubectl get svc -n "$AMP_NS" agent-management-platform-console \
            -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null) || true

        if [[ -n "$nodeport_console" && "$nodeport_console" != "null" ]]; then
            break
        fi

        log_info "Waiting for Console service... (attempt $i/30)"
        sleep 5
    done

    if [[ -n "$nodeport_console" && "$nodeport_console" != "null" ]]; then
        start_socat_verified 3000 "$worker_node" "$nodeport_console" "Console" || \
            log_warning "Failed to setup Console port forwarding"
    else
        log_warning "Could not setup Console port forwarding (service may not be NodePort type)"
    fi
    echo ""

    # Agent Manager Service (8080)
    log_info "Finding Agent Manager NodePort..."
    local nodeport_agent_mgr
    for i in {1..30}; do
        nodeport_agent_mgr=$(kubectl get svc -n "$AMP_NS" agent-management-platform-agent-manager-service \
            -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null) || true

        if [[ -n "$nodeport_agent_mgr" && "$nodeport_agent_mgr" != "null" ]]; then
            break
        fi

        log_info "Waiting for Agent Manager service... (attempt $i/30)"
        sleep 5
    done

    if [[ -n "$nodeport_agent_mgr" && "$nodeport_agent_mgr" != "null" ]]; then
        start_socat_verified 8080 "$worker_node" "$nodeport_agent_mgr" "Agent Manager" || \
            log_warning "Failed to setup Agent Manager port forwarding"
    else
        log_warning "Could not setup Agent Manager port forwarding (service may not be NodePort type)"
    fi
    echo ""
    # Traces Observer (9098)
    log_info "Finding Traces Observer NodePort..."
    local nodeport_traces
    for i in {1..30}; do
        nodeport_traces=$(kubectl get svc -n "$OBSERVABILITY_NS" traces-observer-service \
            -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null) || true

        if [[ -n "$nodeport_traces" && "$nodeport_traces" != "null" ]]; then
            break
        fi

        log_info "Waiting for Traces Observer service... (attempt $i/30)"
        sleep 5
    done

    if [[ -n "$nodeport_traces" && "$nodeport_traces" != "null" ]]; then
        start_socat_verified 9098 "$worker_node" "$nodeport_traces" "Traces Observer" || \
            log_warning "Failed to setup Traces Observer port forwarding"
    else
        log_warning "Could not setup Traces Observer port forwarding"
    fi
    echo ""
    # Data Prepper (21893)
    log_info "Finding Data Prepper NodePort..."
    local nodeport_dataprepper
    for i in {1..30}; do
        nodeport_dataprepper=$(kubectl get svc -n "$OBSERVABILITY_NS" data-prepper \
            -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null) || true

        if [[ -n "$nodeport_dataprepper" && "$nodeport_dataprepper" != "null" ]]; then
            break
        fi

        log_info "Waiting for Data Prepper service... (attempt $i/30)"
        sleep 5
    done

    if [[ -n "$nodeport_dataprepper" && "$nodeport_dataprepper" != "null" ]]; then
        start_socat_verified 21893 "$worker_node" "$nodeport_dataprepper" "Data Prepper" || \
            log_warning "Failed to setup Data Prepper port forwarding"
    else
        log_warning "Could not setup Data Prepper port forwarding"
    fi
    echo ""
    # External Gateway (8443) - already setup by OpenChoreo as LoadBalancer/NodePort
    log_info "Finding External Gateway NodePort..."
    local nodeport_gateway
    for i in {1..30}; do
        nodeport_gateway=$(kubectl get svc -n openchoreo-data-plane gateway-external \
            -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null) || true

        if [[ -n "$nodeport_gateway" && "$nodeport_gateway" != "null" ]]; then
            break
        fi
    ...
    if [[ -n "$nodeport_gateway" && "$nodeport_gateway" != "null" ]]; then
        start_socat_verified 8443 "$worker_node" "$nodeport_gateway" "External Gateway" || \
            log_warning "Failed to setup External Gateway port forwarding"
    else
        log_warning "Could not setup External Gateway port forwarding"
    fi
    echo ""

    log_success "Port forwarding setup complete"

    # Show summary of active port forwards
    echo ""
    log_info "Active port forwards:"
    ps aux | grep "[s]ocat TCP-LISTEN" | awk '{print "  PID " $2 ": " $0}' || true
